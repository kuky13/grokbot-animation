...
engine.morph.x = 0;
activeState = "dictating";
engine.state = "dictating";
configs.dictating.speechMouthScale = 1.8;
engine.manualSpeech = 1;
engine.paused = false;
renderDrippyCharacter(engine, 9000, true);
assert.match(nodes.mouth.getAttribute("d"), / C /, "Paint speech opens a shaped mouth");
assert.ok(Number(nodes.mouthOpen.style.opacity) > 0, "Paint speech shows the inner mouth");
engine.paused = true;
renderDrippyCharacter(engine, 9100, true);
assert.equal(Number(nodes.mouthOpen.style.opacity), 0, "paused Paint speech closes the mouth");
engine.paused = false;
delete configs.dictating.speechMouthScale;
activeState = "idle";
