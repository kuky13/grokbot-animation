...
const REDUCE_MOTION = typeof globalThis.matchMedia === "function"
  ...
  : { matches: false };
const REACTIVE_STATES = new Set(["idle", "listening", "thinking", "happy", "proud", "shy", "curious", "confused", "angry", "excited", "laughing", "playful", "celebrate"]);

// One short reaction, shared by the face and body; delayed parts follow the eyes.
export function expressionPulse(engine, now, delay = 0) {
  const progress = clamp((now - (engine.reactionAt ?? -Infinity) - delay) / 500, 0, 1);
...
export function updateStateTargets(now, config, delta) {
  ...
  const reaction = REDUCE_MOTION.matches ? 0 : expressionPulse(this, now, 160);
  ...
  if (REDUCE_MOTION.matches) {
    ...
    switch (this.state) {
      ...
      case "idle":
        ...
        break;
      case "listening":
        rotation = 2 + 0.6 * Math.sin(0.5 * runtime) + 1.5 * reaction;
        x = 1; y = -1 + 0.5 * Math.sin(0.8 * runtime) + 2.5 * reaction; scaleY = 1.008;
        break;
      case "thinking":
        rotation = -2 + 1.2 * Math.sin(0.35 * runtime) - reaction; x = 1.2 * Math.sin(0.3 * runtime); y = Math.sin(0.6 * runtime);
        break;
      ...
      case "excited": y = -4 * reaction; scaleY = 1 + 0.025 * reaction; eyeScale = 1.05; rotation = 2 * reaction; break;
      ...
      case "suspicious":
        rotation = -3 + Math.sin(0.3 * runtime); x = -1; y = 1; eyeOpen = 0.85;
        break;
      case "angry":
        rotation = 1.2 * reaction; y = 2 + reaction; scaleY = 1 - 0.015 * reaction;
        break;
      case "drowsy":
        ...
        break;
      case "happy": rotation = Math.sin(0.7 * runtime) + 1.5 * reaction; y = -1 - 2 * reaction; scaleY = 1 + 0.012 * reaction; eyeScale = 1.03; break;
      case "curious":
        rotation = 3 + 1.5 * reaction; x = 1 + 1.5 * reaction; y = -1; scaleY = 1.008; eyeScale = 1.04;
        break;
      case "confused": rotation = -2 + 4 * reaction; x = reaction; y = 0.5; eyeOpen = 0.95; break;
      case "bored":
      ...
      case "shy": rotation = -3 - reaction; x = -1.5; y = 2 + reaction; scaleY = 0.99; eyeScale = 0.95; eyeOpen = 0.9; break;
      ...
      case "laughing": { const chuckle = Math.sin(elapsed * 18) * reaction; rotation = 1.5 * chuckle; y = -2 * reaction - Math.abs(chuckle); scaleY = 1 + 0.015 * chuckle; eyeOpen = 0.9; break; }
      case "scared": rotation = reaction; x = -1 - reaction; y = 2; scaleY = 0.97; eyeScale = 1.12; eyeOpen = 1.05; break;
      case "playful": rotation = 3 * reaction; x = 2 * reaction; y = -2 * reaction; eyeScale = 1.04; break;
      case "celebrate": {
        y = -5 * reaction; rotation = 3 * reaction; scaleY = 1 + 0.025 * reaction; eyeScale = 1.06;
        break;
      }
...
export function updateAim(now, config) {
  ...
  switch (this.state) {
    ...
    case "curious": x = direction() * random(0.6, 1) * 15; y = 9 * random(-1, 1); min = 3000; max = 5000; break;
    case "confused": x = direction() * random(0.5, 1) * 15; y = 9 * random(-0.6, 1); min = 3000; max = 5500; break;
...
export function updateGestures(now) {
  ...
  const reacting = now < (this.reactionAt ?? -Infinity) + 700;
  if (!reacting && ["idle", "happy", "excited", "curious", "playful"].includes(this.state) && now >= this.winkNext && !this.blinkQueue.length && this.eyeOpen.x > 0.98) {
    this.winkAt = now;
    this.winkEye = 1 - this.winkEye;
    this.winkNext = now + random(4500, 10000);
  }
  if (REACTIVE_STATES.has(this.state) && now >= this.ambientNext && !this.gesture && !this.spinSpring && !this.blinkQueue.length && now >= this.winkAt + 400 && this.eyeOpen.x > 0.65 && this.morph.x < 0.05) {
    this.reactionAt = now;
    this.ambientNext = now + 700 + random(3000, 7000);
  }
  if (this.gesture) {
